...
\begin{figure}[!htb]
    ...
    \caption{\textbf{Challenge for text segmentation}.}
...
For the experiment, we utilized three type of features including word, tpoic, and punctuation mark. Each word represented in a binary format, where 1 indicates the appearance of the word and 0 for absence. Topics are considered as features since topic models are very effective\cite{kotov2015interpretable,hashimoto2016topic,lu2016modeling} to represent text documents. In this paper, we exploit the Labeled LDA model\cite{kotov2015interpretable} and represent each word in a vector of 2 topics, where the number of topics is experimentaly determined by the model performance. Punctuation mark is one of the most important feature as they indicate the boundary of a sentence, clause and phrase. Therefore, punctuation mark containing one of the symbols \{`.', `,', `!', `?', `:', `;', `-'\} is employed as feature.  
