...
legend(0.77,0.30, c("KNN", "SVM", "NB"), 
       lty=c(1,1,1), col=c('red', 'blue', 'green'), cex=1, lwd=1)

# print AUC values
x1 <- read.csv("/home/mehedi/teana/projects/eCoaching-Text-Segmentation/results/roc/True/roc_LSTM_summit.csv")
x2 <- read.csv("/home/mehedi/teana/projects/eCoaching-Text-Segmentation/results/roc/True/roc_GRU_summit.csv")
roc_obj_lstm <- roc(x1$actual, x1$predict_prob)
auc(roc_obj_lstm)

roc_obj_gru <- roc(x2$actual, x2$predict_prob)
auc(roc_obj_gru)
...
lines(r2$fpr, r2$tpr, col='blue', lty=1, lwd=1)
lines(r3$fpr, r3$tpr, col='green', lty=1, lwd=1)
lines(1-roc_obj_lstm$specificities, roc_obj_lstm$sensitivities, col='cyan', lty=1, lwd=1)
lines(1-roc_obj_gru$specificities, roc_obj_gru$sensitivities, col='magenta', lty=1, lwd=1)
lines(0:1, 0:1, col='black', lty=5, lwd=1)
legend(0.75,0.44, c("KNN", "SVM", "NB", "LSTM", "GRU"), 
       lty=c(1,1,1,1,1), col=c('red', 'blue', 'green', 'cyan', 'magenta'), cex=1, lwd=1)
